...
def is_valid_ip_address(ip_address):
    ...
    if len(section) != 4:
        return False
    try:
        return all(0 <= int(x) <= 255 for x in section)
    except ValueError:
        return False
...
def is_valid_hostname(hostname):
    ...
    i = 0
    while(currently_valid and i < len(hostname)):
        if((i == len(hostname) - 1) or i == 0):
            pass

        if(hostname[i].isalnum() or hostname[i].isalpha() or hostname[i] ==
                "-" or hostname[i] == "."):
            currently_valid = True
        else:
            return False
